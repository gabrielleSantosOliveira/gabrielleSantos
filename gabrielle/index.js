...
// Endpoint para buscar as inscrições do usuário  importante
...
app.delete('/api/inscricao/:id', async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ sucesso: true, mensagem: 'Inscrição cancelada com sucesso.' });
    } catch (error) {
...
// Rota para contar a quantidade de inscrições de um usuário específico Lateral
app.get('/api/contarInscricoesUsuario/:id', async (req, res) => {
    const { id } = req.params;

    try {
        // Contando quantas inscrições existem para o usuário
        const count = await Inscricao.count({
            where: {
                id: id  // Filtra pelo id do usuário
            }
        });
        ...
        res.status(500).json({ message: 'Erro ao contar inscrições' });
    }
});

// Rota para contar o número de inscrições para um curso específico
app.get('/api/contarInscricoesCurso/:id_curso', async (req, res) => {
    const { id_curso } = req.params;

    try {
        // Contando quantas inscrições existem para o curso com o id_curso
        const count = await Inscricao.count({
            where: {
                id_curso: id_curso  // Filtra pelas inscrições do curso específico
            }
        });
